...
| `src/components/panels/FlockLeakPanelContent.tsx` | Every Leak tab string; the side panel and mobile sheet: explainer with the researcher's links, filters, caveat, credit |
| `src/components/map/FlockMapLegend.tsx` | Desktop map key (only what the map draws) carrying the OSM compare switch |
