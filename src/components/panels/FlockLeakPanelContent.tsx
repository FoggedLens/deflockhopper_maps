...
import { LEAK_OSM_DEFAULT_BRAND, compareSeedHolds } from '../../utils/leakCompareDefaults';
...
export const FLOCK_LEAK_COPY = {
  ...
  compareDetail: 'Compare to see which of these devices volunteers have already mapped.',
...
/** The one-line key for reading one map over the other, shown wherever the
 *  compare view is on: under the view picker in the panel and the sheet, and
 *  in place of the mobile peek's description. Spans only: the peek row is
 *  a button. */
export function FlockCompareKey({ className = '' }: { className?: string }) {
  return (
    <span className={`flex flex-wrap gap-x-4 gap-y-1 text-xs text-dark-200 ${className}`} aria-label="How to read the map">
      {KEY.map((k) => (
...
/**
 * The page's main control, first in the panel: the view picker. One line
 * under it says what comparing does, or, while comparing, carries the key
 * and, while the compare seed still holds, what the seed narrowed.
 */
function CompareControl() {
  ...
  const brands = useCameraStore((s) => s.filters.brands);
  const seedHolds = compare && compareSeedHolds({ groups, statuses, brands });

  return (
    <section>
      <FlockCompareControl />
      <div className="mt-3 text-xs text-dark-400 leading-relaxed">
        {compare ? (
          <>
            <FlockCompareKey />
            {seedHolds && <p className="mt-2">{FLOCK_LEAK_COPY.compareSeed}</p>}
          </>
        ) : (
          <p>{FLOCK_LEAK_COPY.compareDetail}</p>
        )}
      </div>
    </section>
  );
...
/** The desktop side panel and the mobile full sheet. The view picker leads,
 *  as the page's main control; then what this is and where it came from,
 *  how to narrow it, and what to do with it. The marks' legend is a button
 *  on the map (FlockLegendControl). */
